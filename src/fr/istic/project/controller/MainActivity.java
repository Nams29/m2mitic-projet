...
public class MainActivity extends Activity {
	...
	/**
	 * Dialog qui propose de sélectionner les dossiers concernés par la recherche
	 */
	public boolean dialogSelection() {
		...
		if (externalDirectoryRemovableAvailable) {
			...
			if (!externalDirectoryDeviceAvailable) itemsPreChecked[0] = itemsPreChecked[2]; 
		}
			
				
		AlertDialog.Builder builder = new AlertDialog.Builder(this);
		builder.setTitle("Rechercher dans...");
		builder.setMultiChoiceItems((CharSequence[]) items.toArray(new CharSequence[items.size()]), itemsPreChecked, new DialogInterface.OnMultiChoiceClickListener() {
		   ...
		   .setPositiveButton("OK !", new DialogInterface.OnClickListener() {
		       public void onClick(DialogInterface dialog, int id) {
		   			...
		   			if (externalDirectoryRemovableAvailable) {
		   				if (externalDirectoryDeviceAvailable) useExternalDirectoriesRemovable = resultats[2];
		   				else useExternalDirectoriesRemovable = resultats[0];
		   			}
		    	   		
		    	    /* PARCOURS DES MEDIAS - PREPARATION */  		
		    		File[] directories = FileUtils.getAllowedDirectories(useApplicationDirectory, 
		    																useExternalDirectoriesDevice, 
		    																useExternalDirectoriesRemovable); // Ajout des dossiers à parcourir
